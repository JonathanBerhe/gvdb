...
func TestDesiredPartition_WaitForLeaderBeforeAdvancing(t *testing.T) {
	...
	step := desiredPartition(sts, gvdbclient.LeaderInfo{LeaderID: 0})
	if step.Done {
		t.Fatalf("expected Done=false")
	}
	if step.Partition == nil || *step.Partition != 2 {
...
func TestDesiredPartition_WalkThroughThreeReplicaRollout(t *testing.T) {
	...
	st1 := desiredPartition(s1, gvdbclient.LeaderInfo{LeaderID: 1})
	...
	st2 := desiredPartition(s2, gvdbclient.LeaderInfo{LeaderID: 1})
	...
	st3 := desiredPartition(s3, gvdbclient.LeaderInfo{LeaderID: 1})
	if *st3.Partition != 1 {
	...
	st4 := desiredPartition(s4, gvdbclient.LeaderInfo{LeaderID: 1})
	if *st4.Partition != 0 {
	...
	st5 := desiredPartition(s5, gvdbclient.LeaderInfo{LeaderID: 1})
