...
void PrintUsage(const char* program_name) {
  std::cout << "Usage: " << program_name << " [options]\n"
            ...
            << "\nExamples:\n"
            << "  # Query node 1 with 32 GB cache\n"
            << "  " << program_name << " --node-id 201 --bind-address 0.0.0.0:50070 \\\n"
            << "    --coordinator coord1:50051,coord2:50051 \\\n"
            << "    --memory-limit-gb 32\n"
            << "\n"
            << "  # Query node 2 with 16 GB cache\n"
            << "  " << program_name << " --node-id 202 --bind-address 0.0.0.0:50071 \\\n"
...
void HeartbeatSender(const std::string& coordinator_address,
  ...
  while (!shutdown_flag.load()) {
    try {
      ...
      node_info->set_node_type(NodeType::NODE_TYPE_QUERY_NODE);
...
int main(int argc, char** argv) {
  ...
  log_config.file_path = args.data_dir + "/logs/query_node.log";
  ...
  gvdb::utils::Logger::Instance().Info("Starting GVDB Query Node");
  ...
  int metrics_port = 9200 + (args.node_id - 200);  // 9201, 9202, etc.
  ...
  try {
    // 1. Create storage layer (for caching segments)
    ...
    gvdb::utils::Logger::Instance().Info("Storage layer initialized (for caching)");

    // 2. Create compute layer (query executor)
    auto query_executor = std::make_shared<gvdb::compute::QueryExecutor>(
        segment_manager.get());
    gvdb::utils::Logger::Instance().Info("Compute layer initialized");

    // 3. Create query node (manages segment loading/eviction)
    size_t memory_limit_bytes = args.memory_limit_gb * 1024ULL * 1024ULL * 1024ULL;
    auto query_node = std::make_unique<gvdb::cluster::QueryNode>(
        segment_manager, query_executor, memory_limit_bytes);
    gvdb::utils::Logger::Instance().Info("QueryNode initialized (cache limit: {} GB)",
                                         args.memory_limit_gb);

    // 4. Create gRPC service (query nodes serve read requests)
    ...
    std::cout << "\n========================================" << std::endl;
    std::cout << "GVDB Query Node Started" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    std::cout << "Cache Directory: " << args.data_dir << std::endl;
    std::cout << "Cache Memory Limit: " << args.memory_limit_gb << " GB" << std::endl;
